...
public class Skeleton extends ActiveEntity {
	...
	protected void moveX(float xAmount){
		...
		CollisionRect newXCollisionRect = new CollisionRect(newX, pos.y, getWidth(), getHeight());
		ArrayList<Entity> collidesWithPlayers = map.rectangleCollidesWithPlayers(newXCollisionRect);
		...
		if(collidesWithPlayers.isEmpty()){
			if(!collidesAhead){
				if(!walkingOffCliff){
					this.pos.x = newX;
			...
			for (Entity player: collidesWithPlayers){
				this.playerInteract((Player) player);
			}
		}
	}
